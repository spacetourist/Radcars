...
const TRACKS = [
  {
    ...
    raceFar: '65-neon-carpet-race-far',
    gridFar: '65b-neon-carpet-grid-far'
  },
  {
    index: 1, id: 'gridlock',
    raceFar: '66-gridlock-carpet-race-far',
    gridFar: '66b-gridlock-carpet-grid-far'
  }
];
...
for (const t of TRACKS) {
  ...
  const meta = await page.evaluate(() => {
    ...
    return {
      ...
      urbanRooftopB: !!(pack.scenery && pack.scenery.urbanRooftopB),
  ...
  if (t.index === 0) {
    ...
    const dual = fl.dualLayer
      ? ` dualLayer alpha=${fl.dualAlpha} uvFrac=${fl.dualUvFrac && fl.dualUvFrac.x},${fl.dualUvFrac && fl.dualUvFrac.y}`
      : '';
    const line = fps.ok
      ? `Pixi Neon Loop ZOOM_FAR(~${Number(fps.zoom).toFixed(2)}) mid-straight: ~${fps.fps.toFixed(1)} FPS over ${fps.elapsed.toFixed(2)}s (${fps.frames} frames). Rooftop soft v5.1 padWu=${fl.padWu} tileScale ${fl.tileScaleAfter}${dual}.`
      : 'Pixi FPS measure failed.';
    writeFileSync(join(OUT, '65-rooftop-soft-fps.txt'), line + '\n');
    report.push({ fps, fpsLine: line });
  }
}

writeFileSync(join(OUT, '65-rooftop-soft-meta.json'), JSON.stringify(report, null, 2));
await browser.close();
console.log('done');
